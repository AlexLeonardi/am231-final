...
for mnist_it = 0:(mnist_num-1)
    ...
    X0_vec = X0(:);
    rand_mat = max(0,randi([0, 4], [m, n])-3);
    Omega = find(rand_mat);
    ...
    X_out = uint8(round(X));
    writematrix(X_out, ['../MNist_csv_80pcentMasked/MNIST' num2str(mnist_it) '.csv']);
    
    [xq,yq] = meshgrid(1:1:n, 1:1:m);
    vq = griddata(x,y,v,xq,yq);
    X_out_grid = uint8(round(max(vq,0)));
    writematrix(X_out_grid, ['../MNist_csv_80pcentMasked_Int/MNIST' num2str(mnist_it) '_Int.csv'])
...
%         [breg_it, j, i, rank(X_k)]
    ...
    writematrix(X_out, ['../MNist_csv_80pcentMasked_RR/MNIST' num2str(mnist_it) '_RR.csv']);
