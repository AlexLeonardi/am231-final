...
for mnist_it = 0:(mnist_num-1)
    ...
    X_out = uint8(round(X));
    writematrix(X_out, ['../MNist_csv_50pcentMasked/MNIST' num2str(mnist_it) '.csv']);
    
    [xq,yq] = meshgrid(1:1:n, 1:1:m);
    vq = griddata(x,y,v,xq,yq);
    X_out_grid = uint8(round(max(vq,0)));
    writematrix(X_out_grid, ['../MNist_csv_50pcentMasked_Int/MNIST' num2str(mnist_it) '_Int.csv'])
